...
async function generateBuild(ctx) {
  ...
  await Promise.all([
    ...
    ...[...ctx._idToInstances.values()].flatMap(
      ...
      await fs__default.ensureDir(node_path.join(reportsDir, key));
      await fs__default.ensureDir(node_path.join(reportsDir, key, "transforms"));
      return await Promise.all([
        ...
        ...Object.entries(env.data.transform).map(([id, info]) => fs__default.writeJSON(
...
class InspectContext {
  constructor(options) {
    ...
    __publicField(this, "_configToInstances", /* @__PURE__ */ new Map());
    __publicField(this, "_idToInstances", /* @__PURE__ */ new Map());
...
class InspectContextVite {
  constructor(id, context, config) {
    ...
    this.config = config;
    __publicField(this, "environments", /* @__PURE__ */ new Map());
  ...
  function setupMiddlewarePerf(ctx2, middlewares) {
    ...
    middlewares.forEach((middleware, index) => {
      ...
      middleware.handle = function(...middlewareArgs) {
        ...
        (_a = ctx2.data.serverMetrics.middleware)[url] ?? (_a[url] = []);
